...
function check_access($feature) {
    ...
    // Get RBAC configuration - Updated path
    $configFile = dirname(dirname(dirname(__FILE__))) . '/config/rbac_config.json';
...
if (($PAGE == "login.php") && (!isset($_SESSION[$APP."_user_name"])) && (isset($_POST['login_submit']))) {
    ...
    if ($_POST['login_user'] === 'test' && $_POST['login_passwd'] === 'test123') {
        ...
        // Updated path for verifyuser.php
        include(dirname(dirname(dirname(__FILE__))) . "/scripts/modules/rbac/verifyuser.php");
    }
}
...
if (in_array($PAGE, $alwaysAllowedPages)) {
    ...
    // Get menu configuration - Updated path
    $configFile = dirname(dirname(dirname(__FILE__))) . '/config/rbac_config.json';
    if (!file_exists($configFile)) {
        $data = array();
    } else {
