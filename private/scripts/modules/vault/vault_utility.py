...
class VaultUtility:
    ...
    def __init__(self, vault_url=None, token=None, env_file_path=None):
        ...
        if not self.token:
            ...
            root_dir = Path(os.path.dirname(os.path.abspath(__file__))).parent.parent.parent.parent
            # Default to private/config/vault.env if no path provided
            env_path = env_file_path if env_file_path else os.path.join(root_dir, 'private', 'config', 'vault.env')
            # Clean up the path
