...
$pythonPath = PROJECT_ROOT . "/shared/venv/bin/python";
if (!file_exists($pythonPath)) {
    $pythonPath = "/usr/bin/python3"; // Fallback to system Python
}

$ldapScript = PROJECT_ROOT . "/shared/scripts/modules/ldap/ldapcheck.py";
...
$scriptDir = dirname($ldapScript);
error_log("Script directory: " . $scriptDir);

// Build command with explicit Python interpreter and working directory
$cmd = sprintf('cd %s && PYTHONPATH=%s %s -u %s %s %s %s',
    escapeshellarg($scriptDir),
    escapeshellarg(PROJECT_ROOT . "/shared/scripts/modules"),
